...
%       comment: if given, a descriptive comment for each trial
    ...
    ledStimStruct = struct( ...
        'identifier', 'ledStim', ....
        'type', 'PWM', ...
        'targetDevices', ["BlueLED"], ...
        'rampUp', 0, ...
        'rampDown', 0, ...
        'isAcquisitionTrigger', false, ...
        'duration', 0, ...
...
% out(1,3)    = 1;                                    % TTL ScanImage: Next File
    ...
    for tNum = 1:length(p)
        ...
        s = {StimulusBlock('childIdxes', [2 6]), ...                    %1
            StimulusBlock('stimParams', cameraTriggerStruct), ...       %2
            StimulusBlock('childIdxes', [4 5], 'childRel', 'seq'), ...  %3
            StimulusBlock('stimParams', acqStartStruct), ...            %4
            StimulusBlock('stimParams', nextFileStruct), ...            %5
            StimulusBlock('childIdxes', [7 8 9], 'startDelay', td.tPre), ... %6
            StimulusBlock('stimParams', qs), ...                        %7
            StimulusBlock('stimParams', ps),...                         %8
            StimulusBlock('stimParams', acqStopStruct, 'startDelay', td.tPre + td.tPost - 5)};%9
        td.data = s;
