...
%% create Component Map select
cmCol = ccCol + 3;
componentMapFilesList = getFilesList(obj.path.componentMaps, '.json');
obj.h.ComponentMapDropDown= uidropdown(grid, ...
    'Editable', 'on', 'Items', [{'Auto'} {'Browse...'} componentMapFilesList{2:end}], ...
    'ValueChangedFcn', @(src, event) obj.callbackLoadConfig(src, event), ...
    'Layout', matlab.ui.layout.GridLayoutOptions( ...
        'Row', length(grid.RowHeight), ...
        'Column', cmCol));

obj.h.saveComponentMapBtn = uibutton(grid, ...
    'ButtonPushedFcn', @(src, event) obj.callbackSaveConfig(src, event), ...
    'Icon', saveIcon, ...
    'Layout', matlab.ui.layout.GridLayoutOptions( ...
        'Row', length(grid.RowHeight), ...
...
componentMapLabel = uilabel(grid, 'Text', 'Protocol Map', ...
    'Layout', matlab.ui.layout.GridLayoutOptions( ...
        'Row', length(grid.RowHeight) - 1, ...
        'Column', [cmCol cmCol+1]));
